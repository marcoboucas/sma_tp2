...
class Argument:
    ...
    def __eq__(self, __o: object) -> bool:
        return self.decision == __o.decision and self.item == __o.item and str(self.comparison_list)==str(__o.comparision_list) and str(self.couple_values_list) == str(__o.couple_values_list)

    def add_premiss_comparison(self, best_criterion_name: CriterionName, worst_criterion_name: CriterionName):
        """Adds a premiss comparison in the comparison list.
        """
        self.comparison_list.append(Comparison(best_criterion_name, worst_criterion_name))
    ...
    def list_supporting_proposals(self, item: Item, preferences: Preferences) -> List[CoupleValue]:
        ...
        supporting_premises = list(
            sorted(
                supporting_premises, key=lambda prem: order_criterion.index(prem.criterion_name)
            )
        )
    ...
    def list_attacking_proposal(self, item: Item, preferences: Preferences) -> List[CoupleValue]:
        ...
        for pref in corresponding_preferences:
            if pref.get_value() in {Value.BAD, Value.VERY_BAD}:
                attacking_premises.append(CoupleValue(pref.get_criterion_name(), pref.get_value()))
        
        # Sort the premises by order of preference
        order_criterion = preferences.get_criterion_name_list()
        attacking_premises = list(
            sorted(
                attacking_premises, key=lambda prem: order_criterion.index(prem.criterion_name)
            )
        )
