...
class ArgumentAgent(CommunicatingAgent, PreferencesAgent):
    ...
    def step(self):
        ...
        while self.has_unread_message_with_performative(MessagePerformative.COMMIT):
            commit_message = self.get_last_unread_message_with_performative(
        ...
        while self.has_unread_message_with_performative(MessagePerformative.PROPOSE):
            propose_message = self.get_last_unread_message_with_performative(
        ...
        while self.has_unread_message_with_performative(MessagePerformative.ACCEPT):
            accept_message = self.get_last_unread_message_with_performative(
        ...
        while self.has_unread_message_with_performative(MessagePerformative.ASK_WHY):
            ask_message = self.get_last_unread_message_with_performative(
    ...
    def generate_argument(
        ...
        past_arguments: List[Argument] = None,
    ) -> Argument:
        ...
        if argument is None:
            ...
            premisses = argument.list_supporting_proposals(
                item=item, preferences=self.preference
            )
            ...
            argument.add_premiss_couple_values(
                criterion_name=selected_premiss.criterion_name,
                value=selected_premiss.value,
            )
            return argument
        ...
        if len(argument.couple_values_list) == 1 and len(argument.comparison_list) == 0:
            ...
            other_items = [
                ...
                if x != item
                and self.preference.get_value(x, couple_value.criterion_name)
                > couple_value.value
            ]
            other_items = list(
                filter(lambda x: x not in already_talked_about, other_items)
            )
            if len(other_items) > 0:
                self.logger.info(
                    "We have a criterion value, If Y has a better alternative O_j, j != i, on c_i"
                )
                other_items = list(
                    sorted(
                        other_items,
                        key=lambda x: self.preference.get_value(
                            item=x, criterion_name=couple_value.criterion_name
                        ),
                        reverse=True,
                    )
                )
                ...
                argument.add_premiss_couple_values(
                    ...
                    self.preference.get_value(
                        other_items[0], couple_value.criterion_name
                    ),
                )
            ...
            if (
                ...
                <= couple_value.value
                and self.preference.get_value(item, couple_value.criterion_name)
                < Value.GOOD
            ):
                self.logger.info(
                    "We have a criterion value, If O_i has a bad evaluation on c_i"
                )
            ...
            if len(criteria):
                self.logger.info(
                    "We have a criterion value, If O_i has bad evaluation on c_j (j!=i) and c_j more important than c_i"
                )
                argument = Argument(False, item)
                argument.add_premiss_comparison(
                    best_criterion_name=criteria[0],
                    worst_criterion_name=couple_value.criterion_name,
                )
                argument.add_premiss_couple_values(
                    criteria[0], self.preference.get_value(item, criteria[0])
                )
                return argument
        ...
        if len(argument.couple_values_list) == 1 and len(argument.comparison_list) == 1:
            ...
            if order.index(comparison.best_criterion_name) > order.index(
                ...
                argument.add_premiss_comparison(
                    ...
                    worst_criterion_name=comparison.best_criterion_name,
                )
                argument.add_premiss_couple_values(
        ...
        logging.info(
            "No further argument your Honor... (nbr items seen: %i)",
